...
class KIter(Iterator):
  ...
  def __iter__(self):
    if self.shuffle:
      self.data.shuffle()
    start = self.start
    assert((self.batch_size % self.num_samples) == 0)
    batch_size_uniq = self.batch_size // self.num_samples
    end = self.end - self.end % batch_size_uniq
    for idx in xrange(start, end, batch_size_uniq):
      rv_batch = self.data.slices(idx, idx + batch_size_uniq)
      yield (np.repeat(rv_batch[0], self.num_samples, axis=1),)
...
def main(args):
    ...
    log_ws = recon.mean(axis=0)
    log_ws_matrix = log_ws.reshape((log_ws.shape[0]//iwae_k, iwae_k))
    ...
    nll_upper_bound = theano.clone(T.dot(log_ws, dummy_vec), replace={dummy_vec: ws_normalized_vector})
    ...
    ((m_s_temp, m_phi_mu_temp, m_phi_sig_temp, m_prior_mu_temp, m_prior_sig_temp, m_z_4_temp, m_z_t), m_updates) =\
        theano.scan(fn=inner_fn,
                    sequences=[m_x_4_temp],
                    outputs_info=[m_s_0, None, None, None, None, None, None])
